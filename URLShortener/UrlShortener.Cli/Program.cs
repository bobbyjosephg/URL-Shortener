...
createCommand.SetHandler(async (destination, path, connectionString) =>
{
    ...
    try
    {
        ...
        RandomKeyGenerator re = new();
         var test =  re.RandomPassword();
        await shortUrlRepository.Create(new ShortUrl(destination, test));
        Console.WriteLine("Shortened URL created." + "     https://mfin.co.on/"+test);
    }
    catch (Exception e)
    {
